...
// Структура шаблона (templates.body): готовый список с задачами.
...
// POST /api/admin/templates — шаблоны создаёт только root (по ТЗ).
func (a *API) handleCreateTemplate(w http.ResponseWriter, r *http.Request) {
	...
	if u.Role != "root" {
		errJSON(w, http.StatusForbidden, "шаблоны создаёт только root")
		return
	}
	var in struct {
	...
	if err := json.Unmarshal(in.Body, &body); err != nil || body.ListName == "" {
		errJSON(w, http.StatusBadRequest, "body: ожидается {list_name, tasks[]}")
		return
	}
	var id int64
...
// GET /api/templates — доступные шаблоны (видны всем активным).
...
func (a *API) handleDeleteTemplate(w http.ResponseWriter, r *http.Request) {
	...
	if u.Role != "root" {
		errJSON(w, http.StatusForbidden, "шаблоны удаляет только root")
		return
	}
	id, err := pathID(r)
	if err != nil {
...
// POST /api/templates/{id}/apply {space_id} — развернуть шаблон в пространстве.
func (a *API) handleApplyTemplate(w http.ResponseWriter, r *http.Request) {
	...
	if err := readJSON(r, &in); err != nil || in.SpaceID == 0 {
		errJSON(w, http.StatusBadRequest, "укажите space_id")
		return
	}
	role := a.spaceRole(r, u.ID, u.IsAdmin(), in.SpaceID)
	if role != "owner" && role != "member" {
		errJSON(w, http.StatusForbidden, "нет прав в пространстве")
		return
	}
	var raw []byte
	if a.DB.Pool.QueryRow(r.Context(), `SELECT body FROM templates WHERE id=$1`, id).Scan(&raw) != nil {
		errJSON(w, http.StatusNotFound, "шаблон не найден")
		return
	}
	listID, err := a.applyTemplate(r.Context(), raw, in.SpaceID, u.ID)
	if err != nil {
		errJSON(w, http.StatusInternalServerError, "не удалось применить шаблон")
		return
	}
	writeJSON(w, http.StatusCreated, map[string]any{"list_id": listID})
}

// applyTemplate разворачивает body шаблона в новый список пространства.
...
// postApprove — что происходит после одобрения нового пользователя:
// доступ в демо-пространство, личное пространство, auto_apply-шаблоны и онбординг-квесты.
func (a *API) postApprove(ctx context.Context, userID int64, username string) {
	// 1. добавляем в демо-пространство (если создано при setup)
	...
	// 4. онбординг-квесты (отключаются: onboarding.quests = "off")
